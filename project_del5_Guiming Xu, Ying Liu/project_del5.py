...
class DataSet:
    ...
    def explore(self):
        ...
        print('Descriptive Information: ',descriptive)
            ...
            #plt.scatter(self.df[self.df.columns[1]], self.df[self.df.columns[2]], alpha=0.5)
            ...
            #plt.plot(self.df[self.df.columns[3]], self.df[self.df.columns[4]])
...
class TimeSeriesDataSet(DataSet):
    ...
    def clean(self):
        ...
        print("TimeSeriesDataset clean function executed successfully")
    ...
    def explore(self,target):
        ...
        #print(self.df.corr())
        print("TimeSeriesDataset explore function executed successfully")
...
class QuantDataSet(DataSet):
    ...
    def clean(self):
        ...
        print( "QuanDataset clean method executed successfully")
    ...
    def explore(self):
        ...
        # Create a boxplot for column 1 and column 2
        data = [self.df[self.df.columns[1]], self.df[self.df.columns[2]]]
        plt.boxplot(data)
        plt.title('Boxplot for Column 1 and Column 2')
        plt.xlabel('Column 1 and Column 2')
        plt.show()

        # Create a histogram of column 2
        plt.hist(self.df[self.df.columns[2]])
        ...
        print( "QuanDataset explore method executed successfully" )
...
############### O(n) = n^2 #################
    ...
    def clean(self):
        ...
        cleanTransactions = [[i for i in nested if i != 'nan'] for nested in transactions] # T(n) = 100      
...
class HeterogeneousDataSet(DataSet):
    ...
    def __init__(self, filenames):
        """
        Create a new heterogeneous dataset instance

        filenames          a list of different datasets filenames
        """
        order_data = int(input("Please input the type of datasets one by one: \n" +
                                "1:QuantData 2:QualData 3:TextData 4:TimeData 5: TransactionData"))
        def pre_quan():
            # prepare quantData list
            quanIndexes = input('Please enter the index/indexes of quantitative datasets that you want to deal with: (in this case: 0 1)')
            quanIndexes_list = list(map(int, quanIndexes.split()))
            quantData = []
            for i in quanIndexes_list:
                quantData.append(QuantDataSet(filenames[i]))
            return quantData
        
        def pre_qual():
            # prepare qualData list
            qualIndexes = input('Please enter the index/indexes of qualitative datasets that you want to deal with: (in this case: 2 3)')
            qualIndexes_list = list(map(int, qualIndexes.split()))
            qualData = []
            for i in qualIndexes_list:
                qualData.append(QualDataSet(filenames[i]))   
            return qualData
        
        def pre_text():
            # prepare textData list
            textIndexes = input('Please enter the index/indexes of text datasets that you want to deal with: (in this case: 4)')
            textIndexes_list = list(map(int, textIndexes.split()))
            textData = []
            for i in textIndexes_list:
                textData.append(TextDataSet(filenames[i])) 
            return textData
        
        def pre_time():
            # prepare TimeSeriesData list
            TimeSeriesIndexes = input('Please enter the index/indexes of TimeSeries datasets that you want to deal with: (in this case: 5)')
            TimeSeriesIndexes_list = list(map(int, TimeSeriesIndexes.split()))
            TimeSeriesData = []
            for i in TimeSeriesIndexes_list:
                TimeSeriesData.append(TimeSeriesDataSet(filenames[i]))  
            return TimeSeriesData
        
        def pre_tran():
            # prepare transactionData list
            transactionIndexes = input('Please enter the index/indexes of transaction datasets that you want to deal with: (in this case: 6)')
            transactionIndexes_list = list(map(int, transactionIndexes.split()))
            transactionData = []
            for i in transactionIndexes_list:
                transactionData.append(TransactionDataSet(filenames[i]))   
            return transactionData
        
        order_data = [int(i) for i in str(order_data)]
        for value in order_data:
            ...
            if value == 3:
                textData = pre_text()
            if value == 4:
                TimeSeriesData = pre_time()
            if value == 5:
                transactionData = pre_tran()
        
        # Datalist consists of different types of datasets lists
        self.Datalist = [quantData, qualData, textData, TimeSeriesData, transactionData]  


    def clean(self):
        """
        Call the clean methods from each of the individual datasets
        """
        # create five lists to store clean datasets lists
        cleanQuandata=[]
        cleanQualdata=[]
        cleanTextdata=[]
        cleanTimeSeriesdata=[]
        ...
        # iterate the Quandataset list
        for i in range(len(self.Datalist[0])):                    
            cleanQuandata.append(self.Datalist[0][i].clean()) # clean the quandata
                 
        # iterate the Qualdataset list
        for i in range(len(self.Datalist[1])):                    
            cleanQualdata.append(self.Datalist[1][i].clean()) # clean the qualdata
        
        # iterate the Textdataset list
        for i in range(len(self.Datalist[2])):                    
            cleanTextdata.append(self.Datalist[2][i].clean()) # clean the textdata
                       
        # iterate the TimeSeriesdataset list
        for i in range(len(self.Datalist[3])):                    
            cleanTimeSeriesdata.append(self.Datalist[3][i].clean()) # clean the TimeSeriesdata
        
        # iterate the Transactiondataset list
        for i in range(len(self.Datalist[4])):  
            cleanTransactiondata.append(self.Datalist[4][i].clean()) # clean the Transactiondata
        
        # cleanDatalist to store all clean datasets
        self.cleanDatalist = [cleanQuandata, cleanQualdata, cleanTextdata, cleanTimeSeriesdata, cleanTransactiondata]
        
                
    def explore(self):
        """
        Call the explore methods from each of the individual datasets
        """   
        play = True
        stop = False
        while play and stop == False:
            Choice = input('Which type of datasets do you want to explore: (Quandataset/Qualdataset/Textdataset/TimeSeriesdataset/Transactiondataset) \n' +
            ...
            elif Choice == 'Quandataset':
                
                # iterate the Quandataset list
                for j in range(len(self.Datalist[0])):                    
                    self.Datalist[0][j].explore() # explore the quandata
                
                print("=========================================================\n")
                        
            elif Choice == 'Qualdataset':
   
                # iterate the Qualdataset list
                for j in range(len(self.Datalist[1])):  
                    num_target = input('Please enter the column name of your target amount: (in this case: loan_amount/reason)')
                    self.Datalist[1][j].explore(num_target) # explore the qualdata 
                
                print("=========================================================\n")

            elif Choice == 'Textdataset':
                
                # iterate the Textdataset list
                for j in range(len(self.Datalist[2])):                    
                    self.Datalist[2][j].explore() # explore the textdata
                
                print("=========================================================\n")
                                            
            elif Choice == 'TimeSeriesdataset':
                
                # iterate the TimeSeriesdataset list
                for j in range(len(self.Datalist[3])): 
                    target = input('Please enter the column name of your target object: (in this case: Occupancy)')
                    self.Datalist[3][j].explore(target) # explore the TimeSeriesdata
                
                print("=========================================================\n")
                                            
            elif Choice == 'Transactiondataset':
                
                # iterate the Transactiondataset list
                for j in range(len(self.Datalist[4])):                    
                    self.Datalist[4][j].output() # explore the Transactiondata
                
                print("=========================================================\n")
                                                   
            else:
                Choice = input('Incorrect option. Please type "Quandataset", "Qualdataset", "Textdataset", "TimeSeriesdataset", or "Transactiondataset" to try again: ')

            # Continue or break?
            answer = input("Would you like to explore Datalists again?: (yes/no)").lower()
            while True:
                if answer == 'yes':
                    play = True
                    break
    ...
    def select(self):
        """
        Select one of the clean constituent datasets 
        """  
        play = True
        stop = False
        
        while play and stop == False:
            Choice = input('Which type of datasets do you want to select: (Quandataset/Qualdataset/Textdataset/TimeSeriesdataset/Transactiondataset) \n' +
                           'If you want to stop please type "Exit"')
            ...
            if Choice == 'Exit':
                ...
                index = int(input('Please enter the index of your desired dataset: (in this case: 0 or 1)'))
                selectDataset = self.cleanDatalist[0][index]                
                print(selectDataset)
                

            elif Choice == 'Qualdataset':
                index = int(input('Please enter the index of your desired dataset: (in this case: 0 or 1)'))
                selectDataset = self.cleanDatalist[1][index]                
                print(selectDataset)

            elif Choice == 'Textdataset':
                index = int(input('Please enter the index of your desired dataset: (in this case: 0)'))
                selectDataset = self.cleanDatalist[2][index]                
                print(selectDataset)
                                            
            elif Choice == 'TimeSeriesdataset':
                index = int(input('Please enter the index of your desired dataset: (in this case: 0)'))
                selectDataset = self.cleanDatalist[3][index]                
                print(selectDataset)
                
            elif Choice == 'Transactiondataset':
                index = int(input('Please enter the index of your desired dataset: (in this case: 0)'))
                selectDataset = self.cleanDatalist[4][index]                
                print(selectDataset)
            ...
            # Continue or break?
            answer = input("Would you like to select again?: (yes/no)").lower()
            while True:
                if answer == 'yes':
                    play = True
                    break
