...
class SimpleKNNClassifier(ClassifierAlgorithm):
    ...
    def split(self):
        ...
        train_size = int(0.7*len(self.X)) # T(n) = 1, S(n) = n(286)
    ...
    def runCrossVal(self, k):
        ...
        for i in range(k):
            ...
            for clf in range(len(self.clfs)):
                ...
                y_pred = current.test(np.asarray(X_test))
...
# Big-O notation for S(n): O(n)
    ...
    def __confusionMatrix(self, y_test, y_pred):
        ...
        classifier_list = ["simplekNN", "simpleTree"]
...
simpleKNN.split()
X_train = simpleKNN.get_X_train().values
y_train = simpleKNN.get_y_train().quality.values
X_test = simpleKNN.get_X_test().values
y_test = simpleKNN.get_y_test().quality.values
simpleKNN.train(X_train, y_train)
y_pred = simpleKNN.test(X_test)

arr = np.array([1, 2, 3, 4, 5])
arr2 = np.array([3, 4, 5, 4, 5])
y_list_test = [y_test,arr]
y_list_pred = [y_pred,arr2]
